你提供的代码示例模拟了三种类型的传感器：温度（temperature）、湿度（humidity）和土壤湿度（soil_moisture）。通过修改或扩展`sensor_types`列表和相应的值生成逻辑，可以模拟更多类型的传感器。以下是一些额外的传感器类型及其可能的值范围，你可以根据需要添加到你的模拟器中：
